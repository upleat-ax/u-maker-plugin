...
| `/u-report [scope] [--only X]` | Phase별 HTML 리포트 생성 (Done/Remaining/Improve 추적). `--only daily`로 IA 일정 조율 Daily Report 생성 (PDCA 5단계 진행 추적). 리포트는 `_reports/{scope}/YYYY-MM-DD/`에 날짜별 저장 | `/u-report [app] --only daily` |
...
```
u-maker-plugin/
├── .claude-plugin/plugin.json        # v3.0.35
...
| IA 일정 조율 Daily Report | `/u-report [app] --only daily` |
| HTML 리포트 전체 생성 | `/u-report [app]` |
